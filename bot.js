...
import {gameBoard, moveCheckerDiv, getBeatPositions, calculateSimpleMoveVariants} from "./game.js"


export default class Bot extends Player {
    makeMove(from, to) {
        gameBoard.board[from].move(gameBoard,from,to);
    ...
    getClosestChecker() {
        ...
        for (let i = 0; i < gameBoard.board.length; i++) {
            if (gameBoard.board[i] !== null && gameBoard.board[i].color === "Black" && gameBoard.board[i].position < closestPosition) {
